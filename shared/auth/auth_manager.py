...
class AuthManager:
    ...
    def __init__(
        self,
        db_path: str = "/app/instance/users.db",
        ...
        if requested_encryption and not SQLCIPHER_AVAILABLE:
            ...
            print("[AUTH] Users database encryption DISABLED (SQLCipher driver missing)")

        self._db = create_encrypted_db(db_path=db_path, encryption_key=db_encryption_key, use_encryption=use_encryption)
        ...
        if secret_key is None:
            ...
            print("[AUTH] WARNING: Generated ephemeral secret key. Set SECRET_KEY in environment for persistence!")
    ...
    def _create_default_users(self):
        ...
        for user in default_users:
            if not self.get_user(user.username):
                ...
                print(
                    f"[AUTH] Created default user: {user.username} (role={user.role.value}, speaker={user.speaker_id})"
                )
    ...
    def authenticate(self, username: str, password: str, ip_address: str | None = None) -> str | None:
        """
