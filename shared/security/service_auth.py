...
class ReplayProtector:
    ...
    def __init__(self, url: str | None = None):
        # Read Redis password from Docker secrets (same as other services)
        redis_password = None
        ...
        if os.path.exists(redis_password_file):
            try:
                ...
                logger.warning("ReplayProtector: Failed to read Redis password file: %s", e)

        # Fallback to env var if no file
        if not redis_password:
            redis_password = os.getenv("REDIS_PASSWORD", "")

        # Build Redis URL with auth
        redis_host = os.getenv("REDIS_HOST", "redis")
        redis_port = os.getenv("REDIS_PORT", "6379")
        if redis_password:
            url = url or f"redis://:{redis_password}@{redis_host}:{redis_port}/0"
        else:
            url = url or os.getenv("REDIS_URL", f"redis://{redis_host}:{redis_port}/0")
        ...
        if redis is not None:
            try:
                ...
                self._client.ping()
                logger.info("ReplayProtector: using Redis backend (host=%s)", redis_host)
            except Exception as e:
                logger.warning("ReplayProtector: Redis unavailable, falling back to in-memory: %s", e)
                self._client = None
        else:
...
class ServiceAuth:
    ...
    def _sign(payload_b64: str, key: bytes) -> str:
        signature = hmac.new(key, payload_b64.encode(), hashlib.sha256).digest()
    ...
    def create_token(self, expires_in: int = 300, aud: str = "internal") -> str:
        ...
        logger.debug(
            f"🔐 JWT CREATE: service={self.service_id} aud={aud} jti={request_id[:12]}... expires_in={expires_in}s"
        )
    ...
    def verify_token(
        self, token: str, allowed_services: list | None = None, expected_aud: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            logger.debug(
                f"🔍 JWT VERIFY START: token_len={len(token)} expected_aud={expected_aud} allowed_services={allowed_services}"
            )
            ...
            logger.debug(
                f"✅ JWT payload decoded: service={payload.get('service_id')} jti={jti_short}... aud={payload.get('aud')}"
            )
            ...
            if expires_at < now:
                logger.error(
                    f"🔴 JWT VERIFY FAILED: Token expired (expired_at={expires_at} now={now} diff={now - expires_at}s ago)"
                )
            ...
            # PHASE 1: Mandatory replay protection
            jti = payload.get("jti") or payload.get("request_id")
            if jti:
                protector = get_replay_protector()
                ttl = max(300, expires_at - now + 60)  # Token lifetime + buffer
                ok, reason = protector.check_and_store(jti, ttl)
                if not ok:
                    logger.error(f"🔴 JWT VERIFY FAILED: Replay attack detected jti={jti[:12]}... reason={reason}")
                    raise ValueError(f"Replay attack detected: {reason}")
                logger.debug(f"✅ JWT replay check passed jti={jti[:12]}...")
...
_replay_protector: ReplayProtector | None = None
...
class ServiceAuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, service_secret, exempt_paths: list = None, enabled: bool = True):
        """
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not token:
            ...
            return JSONResponse({"error": "Missing service token"}, status_code=401)
        ...
        try:
            ...
            return JSONResponse({"error": f"Invalid service token: {e}"}, status_code=401)
