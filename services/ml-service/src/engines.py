...
class VectorEngine:
    ...
    def save(self, path: str):
        """Save FAISS index and metadata using secure JSON serialization (NO PICKLE)."""
        # Save FAISS index (binary format is safe - just float vectors)
        faiss.write_index(self.index, path)

        # Phase 6 Security: Use JSON instead of pickle (prevents RCE)
        # Add HMAC for integrity verification
        import hashlib
        import hmac

        # Get integrity key from environment or use default for dev
        integrity_key = os.getenv("DATA_INTEGRITY_KEY", "nemo-dev-key-change-in-prod").encode()
        ...
        signature = hmac.new(integrity_key, metadata_json.encode(), hashlib.sha256).hexdigest()

        # Save with signature
        with open(path + ".meta.json", "w") as f:
            json.dump({"data": metadata, "signature": signature}, f)

    def load(self, path: str):
        """Load FAISS index and metadata with integrity verification."""
        ...
        if os.path.exists(json_meta_path):
            ...
            # Verify integrity
            metadata_json = json.dumps(wrapper["data"])
            expected_sig = hmac.new(integrity_key, metadata_json.encode(), hashlib.sha256).hexdigest()

            if not hmac.compare_digest(wrapper.get("signature", ""), expected_sig):
                print("⚠️ WARNING: Metadata integrity check failed! File may be corrupted or tampered.")
                # Still load but log the warning - don't break existing deployments

            self.texts = wrapper["data"].get("texts", [])

        # Legacy pickle support REMOVED for security (prevents insecure deserialization)
        elif os.path.exists(path + ".meta"):
            import warnings
            warnings.warn(
                f"SECURITY: Legacy pickle metadata file detected ({path}.meta). "
                "Pickle deserialization has been disabled for security. "
                "Please migrate by loading the pickle file manually with a trusted source, "
                "then saving via VectorEngine.save() to convert to secure JSON format.",
                SecurityWarning,
            )
            print(f"⚠️ SECURITY: Cannot load legacy pickle file '{path}.meta'")
            print("⚠️ Migration required: Convert to JSON format using save() method")
            self.texts = []  # Empty texts, index is still loaded
...
class QueryEngine:
    ...
    def answer_question(self, question: str) -> dict[str, Any]:
        ...
        return {"question": question, "intent": intent, "context": context}
...
class TimeSeriesEngine:
    ...
    def detect_time_col(self) -> str | None:
...
class RecipeEngine:
    ...
    def analyze_churn(self) -> dict[str, Any]:
        ...
        drop_cols = (
            self.cols_by_type.get("ID", []) + self.cols_by_type.get("TIME", []) + self.cols_by_type.get("PII", [])
        )
...
class AnalyticEngine:
    ...
    def analyze_trends(self):
        ...
        chart_recent = {
            ...
            "insight_ref": [5, 6],
            "color_scheme": "success"
            if len(recent_trend) > 1 and recent_trend.iloc[-1] > recent_trend.iloc[0]
            else "warning",
        }
    ...
    def analyze_anomalies(self):
        ...
        if mode == "geo":
            insights = [
                ...
                "📏 **Spread**: Outliers likely represent remote areas or data entry errors in coordinates.",
            ]
        else:
            insights = [
                f"🔴 **Anomalies Detected**: Found **{len(anomalies)} anomalies** ({len(anomalies) / len(df_clean) * 100:.1f}% of {len(df_clean)} total records).",
                "💡 **What are Anomalies?**: These are data points that deviate significantly from normal patterns - could be fraud, errors, or exceptional cases.",
        ...
        chart_data = {
            ...
            "datasets": [
                {
                    "label": "Normal",
                    "data": df_clean[df_clean["anomaly"] == 1]
                    .sample(normal_sample_size, replace=False)
    ...
    def analyze_correlations(self):
        ...
        if len(strong_corrs) > 0:
            ...
            insights.append(
                f"🔗 **Strongest Relationship**: {top_corr[0]} and {top_corr[1]} are {('highly correlated' if top_corr[2] > 0 else 'inversely correlated')} (r={top_corr[2]:.3f})."
            )
            ...
            if positive_corrs:
                insights.append(
                    f"📈 **Positive Drivers**: Found {len(positive_corrs)} positive correlations - when one goes up, the other follows."
                )
            if negative_corrs:
                insights.append(
                    f"📉 **Inverse Relationships**: {len(negative_corrs)} negative correlations detected - potential trade-offs or constraints."
                )

            insights.append(
                f"💡 **Actionable**: Strong correlation between {top_corr[0]} and {top_corr[1]} means optimizing one will likely influence the other."
            )
            insights.append(
                "🎯 **Caution**: Correlation ≠ causation. Further testing needed to confirm causal relationships."
            )
            ...
            if time_sensitive:
                insights.append(
                    f"💰 **Revenue Impact**: {len(time_sensitive)} financial metrics show correlation with other variables."
                )
        else:
            insights.append(
                "🤔 **Weak Correlations**: No strong relationships found between variables (all |r| < 0.3)."
            )
            insights.append(
                "📊 **Independence**: Variables appear largely independent - changes in one won't predictably affect others."
            )
    ...
    def analyze_distributions(self):
        ...
        if abs(skewness) < 0.5:
            insights.append(
                f"⚖️ **Shape**: Distribution is fairly symmetric (skew={skewness:.2f}), similar to normal distribution."
            )
        elif skewness > 0.5:
            insights.append(
                f"📈 **Right-Skewed**: Most values are on the low end with a long tail of high values (skew={skewness:.2f}) - a few outliers pull the average up."
            )
        else:
            insights.append(
                f"📉 **Left-Skewed**: Most values are on the high end with some low outliers (skew={skewness:.2f})."
            )
        ...
        insights.append(
            f"🚨 **Outliers**: {len(outliers)} values ({len(outliers) / len(data) * 100:.1f}%) fall outside the typical range [{lower_fence:,.2f}, {upper_fence:,.2f}]."
        )

        # Practical implications
        if skewness > 1:
            insights.append(
                "💡 **Implication**: Heavy right skew suggests most entities are similar, with a few high performers. Focus on replicating top performers' strategies."
            )
        elif mean_val > median_val * 1.2:
            insights.append(
                f"🎯 **Actionable**: Mean ({mean_val:,.2f}) significantly exceeds median ({median_val:,.2f}) - address low performers to raise median."
            )
        else:
            insights.append("✅ **Healthy**: Distribution is well-balanced without extreme concentration issues.")
    ...
    def analyze_text_clusters(self):
        ...
        insights = [
            ...
            "🔍 **Technique**: Used TF-IDF Vectorization (to capture keywords) combined with K-Means clustering and PCA for visualization.",
        ]
        ...
        for i in range(min(3, num_clusters)):
            ...
            insights.append(
                f"📂 **Cluster {cluster_id + 1}** ({size} items): Dominated by terms like **'{keywords}'**."
            )

        insights.append(
            "💡 **Business Value**: This technique automatically organizes unstructured text into topics, useful for tagging support tickets, categorizing reviews, or spotting emerging themes."
        )
    ...
    def analyze_cohorts(self):
        ...
        for i in range(min(5, len(retention))):
            ...
            chart_data["datasets"].append(
                {"label": cohort_name, "data": data_row, "borderColor": colors[i % len(colors)], "fill": False}
            )
