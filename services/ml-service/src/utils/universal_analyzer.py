...
try:
    from .column_classifier import (
        ColumnClassifier, ColumnProfile, ColumnRole, StatisticalType,
        classify_dataset
    )
except ImportError:
    from utils.column_classifier import (
        ColumnClassifier, ColumnProfile, ColumnRole, StatisticalType,
        classify_dataset
    )
...
class UniversalAnalyzer:
    ...
    def _generate_dataset_summary(self, df: pd.DataFrame, 
                                   profiles: Dict[str, ColumnProfile]) -> Dict[str, Any]:
    ...
    def _calculate_quality(self, df: pd.DataFrame, 
                           profiles: Dict[str, ColumnProfile]) -> Tuple[float, str]:
    ...
    def _generate_key_findings(self, df: pd.DataFrame, 
        ...
        numeric_cols = [name for name, p in profiles.items() 
                       if p.is_usable and p.statistical_type == StatisticalType.CONTINUOUS]
    ...
    def _generate_explanations(self, df: pd.DataFrame,
                                profiles: Dict[str, ColumnProfile],
    ...
    def _explain_target(self, df: pd.DataFrame, profiles: Dict[str, ColumnProfile],
    ...
    def _explain_quality(self, df: pd.DataFrame, profiles: Dict[str, ColumnProfile]) -> str:
        ...
        if issues:
            return (
                f"Data quality considerations: {'; '.join(issues)}. "
                f"These factors may affect analysis accuracy."
            )
        else:
            return "Data quality is good with minimal missing values and no major issues detected."
    
    def _explain_approach(self, df: pd.DataFrame, profiles: Dict[str, ColumnProfile],
                          summary: Dict[str, Any]) -> str:
        ...
        numeric_features = sum(1 for p in profiles.values() 
                              if p.is_usable and p.statistical_type == StatisticalType.CONTINUOUS)
        categorical_features = sum(1 for p in profiles.values()
                                   if p.is_usable and p.statistical_type in 
                                   [StatisticalType.CATEGORICAL, StatisticalType.BOOLEAN])
    ...
    def _generate_chart_data(self, df: pd.DataFrame, 
                              profiles: Dict[str, ColumnProfile]) -> Dict[str, Any]:
    ...
    def _calculate_feature_importance(self, df: pd.DataFrame, 
        ...
        feature_cols = [name for name, p in profiles.items() 
                       if p.is_usable and p.role == ColumnRole.FEATURE]
    ...
    def _get_target_distribution(self, df: pd.DataFrame, target_col: str,
                                  profiles: Dict[str, ColumnProfile]) -> Dict[str, Any]:
    ...
    def _get_numeric_distributions(self, df: pd.DataFrame,
                                    profiles: Dict[str, ColumnProfile]) -> List[Dict]:
        """Get distributions for numeric columns."""
        distributions = []
        
        numeric_cols = [name for name, p in profiles.items()
                       if p.is_usable and p.statistical_type == StatisticalType.CONTINUOUS][:5]
    ...
    def _generate_classification_prediction(self, df: pd.DataFrame,
                                             profiles: Dict[str, ColumnProfile],
                                             target_col: str) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_cols = [name for name, p in profiles.items()
                          if p.is_usable and p.role == ColumnRole.FEATURE
                          and p.statistical_type in [StatisticalType.CONTINUOUS,
    ...
    def _generate_regression_forecast(self, df: pd.DataFrame,
                                        profiles: Dict[str, ColumnProfile],
                                        target_col: str) -> Dict[str, Any]:
        """
    ...
    def _generate_confidence_chart(self, df: pd.DataFrame,
                                    profiles: Dict[str, ColumnProfile],
                                    target_col: str) -> Dict[str, Any]:
        """
        ...
        chart = {
            ...
            'title': f'Prediction Analysis: {target_col}',
            'datasets': []
        }
    ...
    def _generate_next_steps(self, df: pd.DataFrame,
                              profiles: Dict[str, ColumnProfile],
                              summary: Dict[str, Any]) -> List[str]:
