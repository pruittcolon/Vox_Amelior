...
class NormalizationConfig:
    ...
    # Missing values
    missing_representations: list[str] = field(
    ...
    fill_numeric_na: float | None = None  # None = don't fill
    fill_categorical_na: str | None = None  # None = don't fill
...
class DataNormalizer:
    ...
    def __init__(
        self, temp_dir: str | Path | None = None, config: NormalizationConfig | None = None, cache_enabled: bool = True
    ):
    ...
    def process(
        ...
        config = self.config
        if config_override:
            config = NormalizationConfig(**{**vars(self.config), **config_override})
    ...
    def _load_from_cache(self, source_id: str) -> pd.DataFrame | None:
...
def normalize_dataset(source: str | Path | pd.DataFrame, **config_kwargs) -> pd.DataFrame:
    """
...
def _suggest_targets(df: pd.DataFrame, numeric_cols: list[str], categorical_cols: list[str]) -> list[str]:
...
def validate_for_engine(df: pd.DataFrame, engine_name: str, target_column: str | None = None) -> tuple[bool, list[str]]:
    """
