...
try:
    ...
    from .engines.titan_engine import TitanEngine, TITAN_CONFIG_SCHEMA
...
try:
    from .main import load_dataset, UPLOAD_DIR, sample_for_analytics
except ImportError:
...
class StatisticalAnalysisRequest(BaseModel):
    ...
    investment_column: Optional[str] = None
...
def _cleanup_expired_sessions():
    ...
    now = datetime.now()
    expired = [
        sid for sid, data in _titan_sessions.items()
...
def generate_layman_summary(df: pd.DataFrame, results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    id_patterns = ['rownames', 'row_names', 'row_name', 'rowname', 'id', 'index', 'idx',
                   'unnamed', 'pk', 'key', 'uuid', 'record', 'observation', 'obs']
    meaningful_cols = [c for c in df.columns 
                      if not any(p in c.lower() for p in id_patterns)]
    # Also filter unique ID columns (where every value is different)
    meaningful_cols = [c for c in meaningful_cols 
    ...
    if quality_result.get("success"):
        ...
        if score >= 90:
            summary["key_findings"].append(f"✅ **Excellent data quality** ({score}%) - Your data is clean and ready for analysis.")
        elif score >= 70:
            ...
            summary["key_findings"].append(f"⚠️ **Data needs attention** ({score}%) - {missing:.1f}% missing values detected.")
    ...
    if feature_result.get("success") and feature_result.get("feature_scores"):
        ...
        if top_features:
            ...
            if len(top_features) > 1:
                ...
                summary["key_findings"].append(
                    f"📊 Other factors affecting '{target_name}': {', '.join(others)}"
                )
        ...
        summary["key_findings"].append(
            f"📋 **Categorical dataset** - Most columns are text/categories (e.g., Yes/No values). "
            f"Feature importance analysis requires more numeric data."
        )
        if target_col:
            target_name = target_col.replace("_", " ").title()
            summary["key_findings"].append(
    ...
    if n_meaningful < 2:
        summary["what_this_means"] = "This dataset has too few meaningful columns for pattern analysis. Consider adding more data fields."
    elif summary["quality_score"] >= 70 and len(summary["key_findings"]) > 1:
        summary["what_this_means"] = "Your data is in good shape for analysis. The patterns found suggest actionable insights."
    elif len(summary["key_findings"]) > 0:
        summary["what_this_means"] = "Initial patterns have been identified. Consider improving data quality for more reliable insights."
    else:
...
def run_quick_analysis(df: pd.DataFrame, target: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        id_patterns = ['rownames', 'row_names', 'row_name', 'rowname', 'id', 'index', 'idx',
                       'unnamed', 'pk', 'key', 'uuid', 'record', 'observation', 'obs']
        filtered_cols = [c for c in numeric_df.columns 
                        if not any(p in c.lower() for p in id_patterns)]
    ...
    try:
        ...
        skip_patterns = ['rownames', 'row_name', 'id', 'index', 'idx', 'unnamed', 
                        'pk', 'key', 'uuid', 'seq', 'record',
                        'latitude', 'lat', 'longitude', 'lon', 'lng', 'long',
                        'geolocation', 'geo', 'coord', 'location', 'address',
                        'zip', 'zipcode', 'postal', 'phone', 'fax', 'date', 'time',
                        'agency', 'department', 'district', 'region', 'state', 'city',
                        'name', 'description', 'comment', 'note', 'reason']
        ...
        target_keywords = ['target', 'label', 'class', 'outcome', 'result', 'status',
                          'fatal', 'death', 'died', 'survived', 'accident', 'injury',
                          'violation', 'crime', 'fraud', 'churn', 'default', 'approved',
                          'success', 'fail', 'won', 'lost', 'score', 'rating', 'grade',
                          'price', 'cost', 'amount', 'total', 'revenue', 'profit', 'sales',
                          'arrest', 'belt', 'hazmat', 'alcohol', 'commercial',
...
def run_quick_analysis_v2(df: pd.DataFrame, target: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    """
    try:
        from utils.universal_analyzer import analyze_dataset, AnalysisResult
...
class PredictRequest(BaseModel):
    filename: str
    time_column: Optional[str] = None  # Made optional - will auto-detect
    target_column: Optional[str] = None  # Made optional - will auto-detect
    horizon: int = 30
        ...
        try:
            ...
            if historySessionId:
                ...
                historyService.saveRun(
                    ...
                    config=config,
                    score=results.get('best_score') or (current_variants[0].get('cv_score') if current_variants else None)
                )
...
async def run_single_engine(
    ...
    try:
        ...
        if not os.path.exists(file_path):
            raise HTTPException(
                status_code=404,
                detail={"error": f"File not found: {filename}"}
            )
    ...
    try:
        ...
        try:
            ...
            if result is None:
                ...
                result = await asyncio.wait_for(
                    run_in_threadpool(run_engine_sync),
                    timeout=ENGINE_TIMEOUT
                )
        ...
        try:
            ...
            if sessionId:
                ...
                targetColumn = result.get('target_column') or result.get('config', {}).get('target') or request.target_column
...
async def list_premium_engines():
    ...
    engine_info = {
        ...
        'scout': {
            'name': 'Scout Engine',
            'description': 'Data quality assessment and feature profiling',
            'icon': '🔍'
        },
        'oracle': {
            'name': 'Oracle Engine',
...
ALL_ENGINES = {
    ...
    'scout': {'class': ScoutEngine, 'category': 'premium', 'name': 'Scout Drift Detection', 'icon': '🔍', 'priority': 3},
    ...
    'chronos': {'class': ChronosEngine, 'category': 'premium', 'name': 'Chronos Forecasting', 'icon': '⏱️', 'priority': 8},
    'deep_feature': {'class': DeepFeatureEngine, 'category': 'premium', 'name': 'Deep Feature', 'icon': '🧬', 'priority': 9},
    'galileo': {'class': GalileoEngine, 'category': 'premium', 'name': 'Galileo Graph', 'icon': '🔭', 'priority': 10},
    # Core Analytics Engines
    'statistical': {'class': StatisticalEngine, 'category': 'core', 'name': 'Statistical Analysis', 'icon': '📊', 'priority': 11},
    ...
    'predictive': {'class': PredictiveEngine, 'category': 'core', 'name': 'Predictive Analytics', 'icon': '🎱', 'priority': 15},
    # Business Analytics Engines
    'cost_optimization': {'class': CostOptimizationEngine, 'category': 'business', 'name': 'Cost Optimization', 'icon': '💰', 'priority': 16},
    'roi_prediction': {'class': ROIPredictionEngine, 'category': 'business', 'name': 'ROI Prediction', 'icon': '📈', 'priority': 17},
    'spend_pattern': {'class': SpendPatternEngine, 'category': 'business', 'name': 'Spend Pattern', 'icon': '💳', 'priority': 18},
    'budget_variance': {'class': BudgetVarianceEngine, 'category': 'business', 'name': 'Budget Variance', 'icon': '📋', 'priority': 19},
    'customer_ltv': {'class': CustomerLTVEngine, 'category': 'business', 'name': 'Customer LTV', 'icon': '👥', 'priority': 20},
    'profit_margin': {'class': ProfitMarginEngine, 'category': 'business', 'name': 'Profit Margin', 'icon': '💵', 'priority': 21},
    'cash_flow': {'class': CashFlowEngine, 'category': 'business', 'name': 'Cash Flow', 'icon': '💸', 'priority': 22},
    'pricing_strategy': {'class': PricingStrategyEngine, 'category': 'business', 'name': 'Pricing Strategy', 'icon': '🏷️', 'priority': 23},
    'revenue_forecasting': {'class': RevenueForecastingEngine, 'category': 'business', 'name': 'Revenue Forecasting', 'icon': '📊', 'priority': 24},
    'inventory_optimization': {'class': InventoryOptimizationEngine, 'category': 'business', 'name': 'Inventory Optimization', 'icon': '📦', 'priority': 25},
    'market_basket': {'class': MarketBasketAnalysisEngine, 'category': 'business', 'name': 'Market Basket', 'icon': '🛒', 'priority': 26},
    'resource_utilization': {'class': ResourceUtilizationEngine, 'category': 'business', 'name': 'Resource Utilization', 'icon': '⚙️', 'priority': 27},
}
...
class ComprehensiveAnalysisRequest(BaseModel):
    ...
    engines: Optional[List[str]] = None  # Or specify subset
...
async def run_comprehensive_analysis(request: ComprehensiveAnalysisRequest):
    ...
    try:
        ...
        df = load_dataset(file_path)
        logger.info(f"📊 Comprehensive analysis starting on {request.filename} ({len(df)} rows, {len(df.columns)} cols)")
        ...
        for engine_key in engines_to_run:
            ...
            try:
                ...
                engine_result = await asyncio.wait_for(
                    run_in_threadpool(run_single_engine, engine_key, df.copy()),
                    timeout=ENGINE_TIMEOUT
                )
            except asyncio.TimeoutError:
        ...
        if results:
            ...
            overall_summary = {
                ...
                'recommendation': best.get('result', {}).get('summary', {}).get('recommendation', 
                    f"Use {best['engine_name']} for the most reliable analysis of this data."),
                'top_3_engines': [
                    {'name': r['engine_name'], 'icon': r['icon'], 'score': r['utility_score']}
        ...
        logger.info(f"✅ Comprehensive analysis complete: {successful_engines} succeeded, {failed_engines} failed in {total_time:.1f}s")
...
try:
    from .core.smart_iterator import (
        SmartIterator, SmartIterateResult, 
        get_engine_tier, should_prompt_user, get_max_iterations,
        estimate_total_time, ENGINE_TIERS, EngineTier
    )
...
class SmartAnalyzeRequest(BaseModel):
    ...
    max_iterations: Optional[int] = None  # Auto-calculated if not specified
    time_budget_seconds: float = 30.0  # Hard timeout
    explain_level: str = "layman"  # layman | technical | both
    features_to_try: Optional[List[str]] = None  # For manual mode
...
async def preflight_analysis(engine_name: str, filename: str):
    ...
    response = {
        ...
        ) if requires_prompt else (
            f"Ready to analyze with {engine_name.upper()}. "
            f"Will test up to {max_iterations} combinations automatically."
        )
    }
...
async def smart_analyze(engine_name: str, request: SmartAnalyzeRequest):
    ...
    try:
        ...
        if not iterate_result.best:
            raise HTTPException(
                status_code=400,
                detail="No valid predictions found. All feature combinations failed."
            )
...
async def iterate_more(engine_name: str, request: IterateMoreRequest):
    ...
    try:
        ...
        if not session:
            raise HTTPException(
                status_code=404,
                detail="Session not found or expired. Run smart-analyze again."
            )
        ...
        if datetime.now() - session['created_at'] > timedelta(hours=1):
            ...
            raise HTTPException(
                status_code=410,
                detail="Session expired. Run smart-analyze again."
            )
...
async def list_active_sessions():
    ...
    now = datetime.now()
    expired = [
        sid for sid, session in SMART_ITERATE_SESSIONS.items()
...
class PredictionVisualizationRequest(BaseModel):
    ...
    filename: str
    target_column: str
    time_column: Optional[str] = None  # Auto-detect if not provided
        ...
        return convert_to_native({
            ...
            "chart_data": {
                "historical": {
                    "x": historical_x,
                    "y": historical_y,
                    "count": len(historical_y)
                },
                "predicted": {
                    "x": predicted_x,
                    "y": predicted_y,
                    "count": len(predicted_y)
                },
                "confidence_interval": {
...
class FinancialDashboardRequest(BaseModel):
    ...
    filename: str
    engines: List[str] = [
        'cost_optimization', 'roi_prediction', 'spend_patterns',
        'budget_variance', 'profit_margins', 'revenue_forecasting',
        'customer_ltv', 'cash_flow', 'inventory_optimization',
        'pricing_strategy', 'market_basket', 'resource_utilization'
    ]
    currency: str = 'USD'
...
async def get_financial_dashboard(request: FinancialDashboardRequest):
    ...
    try:
        ...
        if not valid_engines:
            raise HTTPException(
                status_code=400,
                detail=f"No valid engines specified. Available: {FINANCIAL_ENGINES_LIST}"
            )
...
def formatSankeyData(results: Dict) -> Dict:
    ...
    try:
        ...
        outflows = cash_flow.get('outflows', spend.get('category_totals', {
            'Salaries': 320000,
            'Marketing': 85000,
...
def formatTreemapData(results: Dict) -> Dict:
    ...
    try:
        ...
        return {
            "categories": categories,
            "chart_type": "treemap",
            "title": "Cost Composition Analysis"
        }
    except Exception as e:
...
def formatForecastData(results: Dict) -> Dict:
    ...
    try:
        ...
        historical = revenue.get('historical', {
...
async def deleteEngineRun(session_id: str, engine_name: str, run_index: int):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Run {run_index} deleted"
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def deleteSession(session_id: str):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Session and all runs deleted"
        }
    except HTTPException:
        raise
    except Exception as e:
    ...
    try:
        ...
        for engineName, runs in request.runs.items():
            for i, run in enumerate(runs):
                ...
                runSummaries.append(f"• {engineName.replace('_', ' ').title()} Run {i+1}: Target='{targetCol}' | {metricsStr}")
