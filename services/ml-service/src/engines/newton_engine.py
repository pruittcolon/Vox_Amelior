...
NEWTON_CONFIG_SCHEMA = {
    "population_size": {
        ...
        "max": 5000,
        "default": 500,
        "description": "Size of genetic population",
    },
    "generations": {
        "type": "int",
        "min": 5,
    ...
    "parsimony_coefficient": {
        ...
        "max": 0.1,
        "default": 0.01,
        "description": "Penalty for complex equations",
    },
}
...
class NewtonEngine:
    ...
    def analyze(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not target_col:
            return {
                ...
                "insights": [
                    "📊 **Newton Engine - Not Applicable**: No suitable numeric target column found. Newton needs a continuous variable to predict (e.g., sales, price, profit). Check if your numeric columns contain currency symbols ($) that need to be cleaned."
                ],
            }
        ...
        if X.shape[1] < 1:
            return {
                ...
                "insights": [
                    f'📊 **Newton Engine - Insufficient Data**: Found target "{target_col}" but no numeric features to build an equation. Need at least 1 numeric feature column. Check if columns contain currency symbols ($) or other formatting that prevents numeric parsing.'
                ],
            }
        ...
        if y.std() < 1e-10:
            return {
                ...
                "insights": [
                    f'📊 **Newton Engine - Constant Target**: The target column "{target_col}" has no variance (all values are the same or nearly identical). Cannot discover a meaningful equation for a constant.'
                ],
            }
        ...
        equation, model, train_score, test_score, complexity = self._discover_equation(
            X_train, X_test, y_train, y_test, feature_names, population_size, generations, max_samples, function_set
        )
    ...
    def _generate_insights(
        ...
        # Discovered equation
        insights.append(f"🔬 **Discovered Equation**:\n   `{target_col} = {equation}`")

        # Check if raw R² was very negative (model is terrible)
        if raw_test_r2 is not None and raw_test_r2 < -0.5:
            insights.append(
                "⚠️ **Poor Model Fit**: The discovered equation performs worse than a simple mean prediction. "
                "This suggests the relationship is highly complex or the features are not predictive. "
                "Consider trying different feature engineering or a non-symbolic approach."
            )
        else:
            # Model performance
            insights.append(
    ...
    def run_premium(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> PremiumResult:
        ...
        variants = [
            Variant(
                rank=1,
                gemma_score=int(r2 * 100),
                cv_score=r2,
                variant_type="equation",
        ...
        return PremiumResult(
            engine_name="newton",
            engine_display_name="Newton Equations",
            engine_icon="📐",
            task_type=TaskType.DISCOVERY,
            target_column=raw.get("target_column"),
            columns_analyzed=raw.get("features", []),
            row_count=len(df),
            variants=variants,
            ...
            config_schema=[
                ...
                for k, v in NEWTON_CONFIG_SCHEMA.items()
            ],
    ...
    def _error_to_premium_result(self, raw, df, config, start):
        ...
        return PremiumResult(
            ...
            best_variant=Variant(
                1, 0, 0.0, "not_applicable" if is_not_applicable else "error", "None", [], message, {}
            ),
            feature_importance=[],
            summary=PlainEnglishSummary(headline, insights[0] if insights else message, recommendation, Confidence.LOW),
...
if __name__ == "__main__":
    ...
    engine = NewtonEngine()
    config = {"target_column": target_col, "generations": generations}
