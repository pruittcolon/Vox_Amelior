...
class TrendEngine:
    ...
    def get_methodology_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "steps": [
                {"step_number": 1, "title": "Time Series Preparation", "description": "Parse temporal column and sort data"},
                {"step_number": 2, "title": "Trend Detection", "description": "Fit linear, polynomial, and exponential trends"},
                {"step_number": 3, "title": "Seasonality Decomposition", "description": "Separate seasonal patterns from noise"},
                {"step_number": 4, "title": "Change Point Detection", "description": "Identify structural breaks in the series"}
            ],
    ...
    def analyze_trends(self, df: pd.DataFrame, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not value_cols:
            ...
            value_cols = [c for c in df.select_dtypes(include=[np.number]).columns.tolist() 
        ...
        return {
            ...
            'metadata': {
                ...
                'time_range': {
                    'start': str(df_sorted.index.min()),
                    'end': str(df_sorted.index.max())
                }
            }
        }
        ...
        try:
            if isinstance(ts.index, pd.DatetimeIndex):
                ...
                seasonal_component = pd.Series(
                    [seasonality.get(d, 0) for d in ts.index.dayofweek],
                    index=ts.index
                )
            else:
