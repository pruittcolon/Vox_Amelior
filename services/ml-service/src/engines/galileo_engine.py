...
GALILEO_CONFIG_SCHEMA = {
    ...
    "similarity_threshold": {
        ...
        "max": 0.9,
        "default": 0.5,
        "description": "Edge creation threshold",
    },
...
class GalileoEngine:
    ...
    async def analyze_async(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not TORCH_GEOMETRIC_AVAILABLE:
            return {
                ...
                "message": "torch_geometric is not installed - GNN functionality unavailable",
                "insights": [
                    "📊 **Galileo Engine**: torch_geometric package not installed. Install with: pip install torch-geometric"
                ],
                "used_gpu": False,
            }
        ...
        try:
            ...
            if not target_col:
                return {
                    ...
                    "status": "not_applicable",
                    "insights": [
                        "📊 **Galileo Engine - Not Applicable**: This engine requires a target column (labels) for node classification. No suitable target column was detected."
                    ],
                    "used_gpu": False,
                }
    ...
    def _detect_target_column(self, df: pd.DataFrame, hint: str | None) -> str | None:
    ...
    def run_premium(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> PremiumResult:
        ...
        variants = [
            Variant(
                rank=1,
                gemma_score=int(accuracy * 100),
                cv_score=accuracy,
                variant_type="graph_model",
                model_name=f"Graph Convolutional Network{gpu_indicator}",
        ...
        return PremiumResult(
            engine_name="galileo",
            engine_display_name="Galileo Graph AI",
            engine_icon="🌐",
            task_type=TaskType.GRAPH_ANALYSIS,
            target_column=raw.get("target_column"),
            columns_analyzed=raw.get("feature_names", []),
            row_count=len(df),
            variants=variants,
            ...
            config_schema=[
                ...
                for k, v in GALILEO_CONFIG_SCHEMA.items()
            ],
    ...
    def _error_to_premium_result(self, raw, df, config, start):
        ...
        return PremiumResult(
            ...
            feature_importance=[],
            summary=PlainEnglishSummary(
                "Graph analysis failed", raw.get("message", "Error"), "Check data format.", Confidence.LOW
            ),
...
if __name__ == "__main__":
    ...
    config = {"target_column": target_col, "epochs": epochs}
