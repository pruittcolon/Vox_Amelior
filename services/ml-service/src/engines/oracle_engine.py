...
ORACLE_CONFIG_SCHEMA = {
    ...
    "significance": {"type": "float", "min": 0.01, "max": 0.1, "default": 0.05, "description": "P-value threshold for significance"},
    "test": {"type": "select", "options": ["ssr_ftest", "ssr_chi2test", "lrtest"], "default": "ssr_ftest", "description": "Statistical test type"},
}
...
class OracleEngine:
    ...
    def analyze(self, df: pd.DataFrame, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        insights = self._generate_insights(
            numeric_cols, causal_relationships, max_lag, test_type
        )
    ...
    def _granger_causality_analysis(
        ...
        max_lag: int, significance: float, test_type: str
    ...
    def _generate_insights(
        ...
        causal: List[Dict], max_lag: int, test_type: str
    ) -> List[str]:
    ...
    def run_premium(self, df: pd.DataFrame, config: Optional[Dict[str, Any]] = None) -> PremiumResult:
        ...
        if not variants:
            variants.append(Variant(
                rank=1, gemma_score=50, cv_score=0.0, variant_type="no_causality",
                model_name="Granger Test", features_used=[],
                interpretation="No causal relationships detected", details={}
            ))
        ...
        return PremiumResult(
            ...
            summary=PlainEnglishSummary(headline=headline, explanation=explanation, 
                                        recommendation="Use causal features for interventions.", confidence=confidence),
            ...
            config_schema=[ConfigParameter(k, v['type'], v['default'], 
                          ...
                          v.get('description', '')) for k, v in ORACLE_CONFIG_SCHEMA.items()],
    ...
    def _error_to_premium_result(self, raw, df, config, start):
        import time
        return PremiumResult(
            engine_name="oracle", engine_display_name="Oracle Causality", engine_icon="🔗",
            ...
            feature_importance=[],
            summary=PlainEnglishSummary("Analysis failed", raw.get('message', 'Error'), "Check data format.", Confidence.LOW),
