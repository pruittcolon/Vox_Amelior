...
class RAGEvaluationEngine:
    ...
    def get_engine_info(self) -> dict[str, str]:
        ...
        return {"name": "rag_evaluation", "display_name": "RAG Evaluation", "icon": "🎯", "task_type": "detection"}
            ...
            "steps": [
                {
                    ...
                    "description": "Calculate Precision@k, Recall@k, MRR, nDCG",
                },
                {
                    "step_number": 2,
                    "title": "Generation Evaluation",
                    "description": "Assess faithfulness and answer relevance",
                },
                {
                    "step_number": 3,
                    "title": "End-to-End Metrics",
                    "description": "Measure overall helpfulness and latency",
                },
                {
                    "step_number": 4,
                    "title": "Summary Generation",
                    "description": "Aggregate metrics and generate insights",
                },
            ],
            "limitations": ["Ground truth required for retrieval metrics", "LLM judge may have biases"],
            "assumptions": [
                "Test cases have relevant documents annotated",
                "RAG responses include retrieved documents",
            ],
        }
    ...
    def analyze(self, df: pd.DataFrame, config: dict | None = None) -> dict[str, Any]:
        ...
        if "query" in df.columns and "retrieved_docs" in df.columns:
            ...
            rag_responses = (
                df[["query", "retrieved_docs", "generated_answer"]].to_dict("records")
    ...
    def _evaluate_retrieval(self, test_set: list[dict], rag_responses: list[dict], k_values: list[int]) -> dict:
        ...
        # Aggregate metrics
        for k in k_values:
            metrics["precision_at_k"][f"p@{k}"] = (
                float(np.mean(precision_at_k_scores[k])) if precision_at_k_scores[k] else 0.0
            )
    ...
    def _evaluate_generation(self, test_set: list[dict], rag_responses: list[dict], config: dict) -> dict:
        ...
        metrics = {"faithfulness": 0.0, "answer_relevance": 0.0, "hallucination_rate": 0.0, "correctness": 0.0}
    ...
    def _evaluate_end_to_end(self, test_set: list[dict], rag_responses: list[dict]) -> dict:
        ...
        metrics = {"average_latency_ms": 0.0, "helpfulness_score": 0.0, "conciseness_score": 0.0}
    ...
    def _generate_eval_summary(self, results: dict) -> dict:
        ...
        summary = {"overall_grade": "N/A", "strengths": [], "weaknesses": [], "recommendations": []}
