...
TITAN_CONFIG_SCHEMA = {
    ...
    "n_bootstrap": {"type": "int", "min": 10, "max": 500, "default": 100, "description": "Bootstrap samples for stability selection"},
    "n_variants": {"type": "int", "min": 1, "max": 50, "default": 10, "description": "Number of analysis variants to generate"},
    "stability_threshold": {"type": "float", "min": 0.5, "max": 1.0, "default": 0.8, "description": "Feature stability threshold (0-1)"},
    "holdout_ratio": {"type": "float", "min": 0.0, "max": 0.5, "default": 0.0, "description": "Holdout set ratio for validation (0 = disabled)"},
...
def _create_model_configs(use_gpu: bool = False):
    ...
    """
    configs = [
        {"name": "RandomForest", "classifier": RandomForestClassifier, "regressor": RandomForestRegressor, 
         "params": {"n_estimators": 100, "max_depth": 10, "random_state": 42}, "gpu": False},
        {"name": "GradientBoosting", "classifier": GradientBoostingClassifier, "regressor": GradientBoostingRegressor,
         "params": {"n_estimators": 100, "max_depth": 5, "random_state": 42}, "gpu": False},
        {"name": "DecisionTree", "classifier": DecisionTreeClassifier, "regressor": DecisionTreeRegressor,
         "params": {"max_depth": 10, "random_state": 42}, "gpu": False},
    ]
...
class TitanEngine:
    ...
    def _analyze_internal(self, df: pd.DataFrame, config: Dict[str, Any], 
                          model_configs: List[Dict], gpu_acquired: bool) -> Dict[str, Any]:
        """
        ...
        stable_features, feature_importance = self._stability_selection(
            X, y, feature_names, task_type, 
            n_bootstrap=n_bootstrap,
            threshold=config.get('stability_threshold', 0.8)
        )
        logger.info(f"[TITAN] Stability Selection complete: {len(stable_features)} stable features, {time.time() - step_start:.2f}s")
        ...
        variants = self._generate_variants(
            df_train, target_col, task_type, feature_names, 
            stable_features, feature_importance, n_variants, config
        )
        ...
        provenance = self._build_provenance(
            df_train, target_col, task_type, config, nested_cv_results, 
            stable_features, feature_importance, holdout_results
        )
        
        # 14. Generate insights (with GPU indicator)
        insights = self._generate_insights(
            task_type, target_col, nested_cv_results, 
            stable_features, feature_importance, final_score, impact_insights,
        ...
        feature_importance_list = self._convert_feature_importance(
            consolidated_fi, stable_features, target_col
        )
    ...
    def _analyze_feature_impact(self, df: pd.DataFrame, target_col: str, features: List[str], task_type: str) -> List[str]:
        ...
        for feat in features[:3]: # Analyze top 3 stable features
            try:
                if task_type == 'regression':
                    ...
                    strength = abs(corr)
                    if strength > 0.5: qual = "Strongly"
                    elif strength > 0.3: qual = "Moderately"
                    else: qual = "Slightly"
                    ...
                    if df[feat].dtype == 'object' or df[feat].nunique() < 20:
                        ...
                        means = df.groupby(feat)[target_col].apply(lambda x: pd.Categorical(x).codes.mean() if not pd.api.types.is_numeric_dtype(x) else x.mean())
                        best_val = means.idxmax()
                        worst_val = means.idxmin()
                        
                        impacts.append(f"      • **{feat}**: Highest '{target_col}' at '{best_val}', Lowest at '{worst_val}'")
                    else:
                        # Continuous feature in classification
                        # Compare means of feature for each class
                        means = df.groupby(target_col)[feat].mean()
                        impacts.append(f"      • **{feat}**: Average value is {means.max():.2f} for class '{means.idxmax()}' vs {means.min():.2f} for class '{means.idxmin()}'")
            except Exception:
    ...
    def _generate_insights(
        self, task_type: str, target_col: str,
        nested_cv: Dict, stable_features: List[str],
        feature_importance: Dict, final_score: float,
        ...
        if stable_features:
            ...
            insights.append(f"   **Top 3 Robust Features & Impact:**")
    ...
    def _detect_target_column(self, df: pd.DataFrame, profiles: Dict, hint: Optional[str]) -> Optional[str]:
        ...
        skip_keywords = ['rowname', 'row_name', 'index', 'idx', 'id', 'key', 'pk', 'uuid', 
                         'record', 'row_id', 'rowid', 'obs', 'observation', 'unnamed',
                         'district', 'county', 'state', 'country', 'region', 'name', 'school']
        ...
        target_keywords = ['target', 'label', 'class', 'y', 'output', 'outcome', 'medv', 
                          'price', 'quality', 'revenue', 'sales', 'profit', 'score', 
                          'rating', 'value', 'amount', 'total', 'result', 'math', 'read',
                          'fatal', 'survived', 'churn', 'default', 'fraud']
    ...
    def _detect_task_type(self, df: pd.DataFrame, target_col: str, profiles: Dict, hint: Optional[str]) -> str:
    ...
    def _prepare_data(self, df: pd.DataFrame, target_col: str) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        ...
        for col in feature_cols:
            ...
            if df[col].dtype == 'bool' or (pd.api.types.is_numeric_dtype(df[col]) and df[col].dropna().isin([0, 1, True, False]).all() and df[col].nunique() <= 2):
        ...
        if len(feature_data) == 0:
            raise ValueError(f"No usable features found for prediction. Available columns: {feature_cols}. Excluded as IDs: {excluded_ids}")
    ...
    def _train_final_model(self, X: np.ndarray, y: np.ndarray, task_type: str) -> Tuple[Any, float]:
        ...
        try:
            scores = cross_val_score(model, X, y, cv=cv_folds, scoring='accuracy' if task_type == 'classification' else 'neg_mean_absolute_error')
    ...
    def _generate_variants(
        ...
        feature_names: List[str], stable_features: List[str],
        feature_importance: Dict[str, float], n_variants: int,
        config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        variants.append({
            ...
            'raw_insights': [f"Uses {len(stable_features)} stable features identified through bootstrap stability selection"]
        })
        ...
        for n_features in [3, 5, 7, 10]:
            ...
            if len(subset) >= 2:
                variants.append({
                    ...
                    'raw_insights': [f"Simplified model with only the {n_features} most important features for interpretability"]
                })
        ...
        for variant in variants:
            try:
                ...
                logging.info(f"[TITAN DEBUG] Variant {variant.get('model_type')}: features_used={variant_features}, mapped_indices={feature_indices}")
                ...
                if len(feature_indices) >= 1:
                    ...
                    if task_type == 'classification':
                        ...
                        logging.info(f"[TITAN DEBUG] MAE={mae:.4f}, y_std={y_std:.4f}, relative_error={relative_error:.4f}, cv_score={cv_score:.4f}")
                    ...
                    logging.warning(f"[TITAN DEBUG] No feature indices found for variant!")
...
"""
    ...
    def _validate_holdout(
        self, df_train: pd.DataFrame, df_holdout: pd.DataFrame,
        target_col: str, stable_features: List[str], 
        task_type: str, model
    ) -> Dict[str, Any]:
        """
    ...
    def _build_provenance(
        self, df: pd.DataFrame, target_col: str, task_type: str,
        config: Dict[str, Any], nested_cv: Dict, stable_features: List[str],
        feature_importance: Dict[str, float], holdout_results: Optional[Dict]
    ) -> Dict[str, Any]:
        """
    ...
    def _consolidate_onehot_features(
        self,
        raw_importance
    ) -> Dict[str, any]:
        """
    ...
    def _convert_feature_importance(
        ...
        for feat, info in consolidated.items():
            ...
            is_stable = feat in stable_features or any(
                sf.startswith(f"{feat}_") for sf in stable_features
            )
            # Determine impact direction based on importance value
            impact = "positive" if importance > 0.5 else "mixed"
            
            explanation = translator.translate_feature(
                feat, stability, is_stable, target_col
            )
    ...
    def _build_summary(
        ...
        if stable_count < total_features * 0.3:
            ...
            recommendation = "Model performance is moderate. Try adding more relevant features or exploring alternative model types."
        elif score < 0.85:
    ...
    def _build_explanation(self, raw_result: Dict) -> TechnicalExplanation:
        ...
        return TechnicalExplanation(
            ...
            methodology_url=methodology.get('reference_url', 'https://scikit-learn.org/stable/auto_examples/model_selection/plot_nested_cross_validation_iris.html'),
    ...
    def _convert_holdout(self, raw_holdout: Optional[Dict]) -> Optional[HoldoutResult]:
        ...
        return HoldoutResult(
            ...
            holdout_ratio=raw_holdout.get('holdout_samples', 0) / max(1, raw_holdout.get('train_samples', 1) + raw_holdout.get('holdout_samples', 1)),
        ...
        if 'error' in result:
            ...
            if variants:
                ...
                for i, v in enumerate(variants[:5], 1):  # Show top 5
                    ...
                    print(f"     Model: {v['model_type']} | Features: {v['feature_count']} | Score: {score_str}{gemma_str}")
