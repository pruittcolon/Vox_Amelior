...
class ClusteringEngine:
    ...
    def get_methodology_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "steps": [
                {"step_number": 1, "title": "Feature Selection", "description": "Identify numeric features via schema intelligence"},
                {"step_number": 2, "title": "Scaling", "description": "Standardize features for distance-based algorithms"},
                {"step_number": 3, "title": "Optimal K Selection", "description": "Use elbow method and silhouette scores"},
                {"step_number": 4, "title": "Clustering", "description": "Apply selected algorithm (K-Means, DBSCAN, Hierarchical)"},
    ...
    def cluster(self, df: pd.DataFrame, config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if not config.get('skip_profiling', False):
            ...
            numeric_cols = [col for col, prof in profiles.items() 
    ...
    def _generate_pca_coordinates(self, X_scaled: np.ndarray, labels: np.ndarray, feature_names: List[str] = None) -> Dict[str, Any]:
        """
            ...
            if feature_names:
                for pc_idx in range(n_components):
                    ...
                    feature_contributions = [
                        {'feature': feature_names[i], 'loading': float(loadings[i]), 'abs_loading': abs(float(loadings[i]))}
