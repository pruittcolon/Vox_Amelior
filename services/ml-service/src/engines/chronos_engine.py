...
class ChronosEngine:
    ...
    def analyze(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if run_all_variants:
            return self._run_all_variants(prophet_df, time_col, target_col, forecast_periods, freq, confidence_interval)

        # 6. Train model and generate forecast (single default variant)
        model, forecast, metrics = self._generate_forecast(prophet_df, forecast_periods, freq, confidence_interval)

        # 6. Generate insights
        insights = self._generate_insights(time_col, target_col, prophet_df, forecast, metrics, forecast_periods)
    ...
    def _detect_target_column(
        self, df: pd.DataFrame, profiles: dict, hint: str | None, exclude: list[str]
    ) -> str | None:
    ...
    def _run_all_variants(
        ...
        for variant_config in PROPHET_VARIANTS:
            ...
            try:
                model, forecast, metrics = self._generate_forecast(
                    prophet_df, forecast_periods, freq, confidence_interval, variant_config=variant_config
                )
        ...
        return {
            ...
            "insights": [
                ...
                f"🏆 **Best Model**: {best_variant['name']} with {best_variant['metrics']['accuracy']:.10f}% accuracy"
                if best_variant
                else "No successful variants",
    ...
    def _generate_forecast(
        ...
        if len(test_df) > 0:
            ...
            test_merged = test_df.merge(forecast[["ds", "yhat", "yhat_lower", "yhat_upper"]], on="ds", how="inner")
                ...
                if non_zero_mask.any():
                    mape = (
                        np.mean(
                            np.abs(
                                (
                                    test_merged.loc[non_zero_mask, "y"].values
                                    - test_merged.loc[non_zero_mask, "yhat"].values
                                )
                                / test_merged.loc[non_zero_mask, "y"].values
                            )
                        )
        ...
        metrics = {"mae": mae, "rmse": rmse, "mape": mape, "train_size": len(train_df), "test_size": len(test_df)}
    ...
    def _generate_insights(
        ...
        if len(future_forecasts) > 0:
            ...
            insights.append("🔮 **Future Forecast**:")
            insights.append(
    ...
    def run_premium(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> PremiumResult:
        ...
        try:
            ...
            raw = {"error": "Analysis failed", "message": str(e)}
        ...
        result = PremiumResult(
            engine_name="chronos",
            engine_display_name="Chronos Forecast",
            engine_icon="⏰",
            ...
            config_schema=[
                ...
                for k, v in CHRONOS_CONFIG_SCHEMA.items()
            ],
    ...
    def _error_to_premium_result(self, raw, df, config, start):
        ...
        result = PremiumResult(
            ...
            feature_importance=[],
            summary=PlainEnglishSummary(
                "Forecasting failed", raw.get("message", "Error"), "Provide time series data.", Confidence.LOW
            ),
...
if __name__ == "__main__":
    ...
    config = {"time_column": time_col, "target_column": target_col, "forecast_periods": forecast_periods}
