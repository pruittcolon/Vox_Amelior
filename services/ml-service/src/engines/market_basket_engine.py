...
class MarketBasketAnalysisEngine:
    ...
    def get_config_schema(self) -> List[ConfigParameter]:
        return [
            ConfigParameter(name="transaction_column", type="select", default=None, range=[], description="Transaction/basket identifier"),
            ConfigParameter(name="item_column", type="select", default=None, range=[], description="Item/product column"),
            ConfigParameter(name="min_support", type="float", default=0.01, range=[0.001, 0.5], description="Minimum support threshold")
        ]
    ...
    def _generate_insights(self, results: Dict, trans_col: Optional[str] = None, item_col: Optional[str] = None) -> List[str]:
        ...
        if results['rules']:
            ...
            insights.append(
                f"💡 Recommendation: Bundle '{top['antecedent']}' and '{top['consequent']}' "
                f"for a cross-sell promotion"
            )
