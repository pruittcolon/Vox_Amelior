...
class CashFlowEngine:
    ...
    def get_engine_info(self) -> dict[str, str]:
        return {"name": "cash_flow", "display_name": "Cash Flow Analysis", "icon": "💸", "task_type": "forecasting"}

    def get_config_schema(self) -> list[ConfigParameter]:
        return [
            ConfigParameter(
                name="inflow_column", type="select", default=None, range=[], description="Cash inflow column"
            ),
            ConfigParameter(
                name="outflow_column", type="select", default=None, range=[], description="Cash outflow column"
            ),
    ...
    def get_requirements(self) -> EngineRequirements:
        return EngineRequirements(
            ...
            optional_semantics={
                "inflow": [SemanticType.NUMERIC_CONTINUOUS],
                "outflow": [SemanticType.NUMERIC_CONTINUOUS],
            },
            required_entities=[],
            preferred_domains=[],
            applicable_tasks=[],
    ...
    def analyze(self, df: pd.DataFrame, config: dict | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if not config.get("skip_profiling", False):
                ...
                amt_col = config.get("amount_column") or self._detect_column(
                    df, ["amount", "value", "transaction"], None
                )
    ...
    def _generate_insights(self, results: dict, date_col: str = None, amt_col: str = None) -> list[str]:
        ...
        if summary["burn_rate"] > 0:
            ...
            insights.append(f"🔥 Monthly Burn Rate: ${summary['burn_rate']:,.2f} (Runway: {runway_str} months)")
        else:
