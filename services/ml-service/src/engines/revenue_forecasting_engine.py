...
class RevenueForecastingEngine:
    ...
    def get_config_schema(self) -> List[ConfigParameter]:
        return [
            ConfigParameter(name="revenue_column", type="select", default=None, range=[], description="Column containing revenue"),
            ConfigParameter(name="date_column", type="select", default=None, range=[], description="Column containing dates"),
            ConfigParameter(name="forecast_periods", type="int", default=12, range=[1, 52], description="Periods to forecast ahead")
        ]
        ...
        try:
            ...
            if not config.get('skip_profiling', False):
                ...
                rev_col = self._smart_detect(
                    df, profiles,
                    keywords=['revenue', 'sales', 'amount', 'income'],
    ...
    def _generate_insights(self, results: Dict, date_col: str = None, rev_col: str = None) -> List[str]:
        ...
        if 'forecast' in results:
            f = results['forecast']
            insights.append(
                f"📈 Forecasted Revenue: ${f['total_forecast_revenue']:,.2f} over next 12 months"
            )
            
        if 'growth' in results:
            g = results['growth']
            insights.append(
                f"🚀 Historical Growth: {g['cagr']:.1f}% CAGR"
            )
