...
class StatisticalEngine:
    ...
    def get_engine_info(self) -> dict[str, str]:
        ...
        return {"name": "statistical", "display_name": "Statistical Analysis", "icon": "📊", "task_type": "detection"}
            ...
            "steps": [
                {
                    ...
                    "description": "Profile columns to identify numeric and categorical variables",
                },
                {
                    "step_number": 2,
                    "title": "Descriptive Statistics",
                    "description": "Calculate mean, median, std, skewness, kurtosis",
                },
                {
                    "step_number": 3,
                    "title": "Correlation Analysis",
                    "description": "Compute Pearson, Spearman, and Kendall correlations",
                },
                {
                    "step_number": 4,
                    "title": "Hypothesis Testing",
                    "description": "Run normality tests, ANOVA, and regression analysis",
                },
            ],
    ...
    def analyze(self, df: pd.DataFrame, config: dict[str, Any]) -> dict[str, Any]:
        ...
        if not config.get("skip_profiling", False):
            ...
            results = {
                ...
                "summary": {},
                "visualizations": [],
                "metadata": {"rows": len(df), "columns": len(df.columns), "analysis_types": analysis_types},
            }
        else:
            results = {
    ...
    def _descriptive_statistics(self, df: pd.DataFrame, config: dict) -> dict:
        ...
        return {"numeric": stats_dict, "categorical": categorical_stats}
    ...
    def _regression_analysis(self, df: pd.DataFrame, config: dict) -> dict:
        ...
        return {
            ...
            "polynomial": {"r_squared": float(poly_r2), "degree": 2},
            "target": target,
            "features": features,
            "sample_size": len(X),
        }
    ...
    def _detect_outliers(self, data: pd.Series) -> dict:
        ...
        return {
            ...
            "z_score_method": {"count": int(len(z_outliers)), "percentage": float(len(z_outliers) / len(data) * 100)},
        }
    ...
    def _generate_summary(self, results: dict, df: pd.DataFrame) -> dict:
        ...
        if "anova" in results:
            if isinstance(results["anova"], dict):
                significant_anova = [
                    k for k, v in results["anova"].items() if isinstance(v, dict) and v.get("significant")
                ]
            else:
                significant_anova = []
