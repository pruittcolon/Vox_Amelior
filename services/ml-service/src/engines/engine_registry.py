...
def _build_registry() -> dict[str, EngineInfo]:
    ...
    try:
        ...
        registry["salesforce_churn"] = EngineInfo(
            name="salesforce_churn",
            display_name="Salesforce Churn Prediction",
            description="Account health scoring and churn risk prediction",
            category=EngineCategory.FINANCIAL,
            engine_class=SalesforceChurnEngine,
            priority=180,
            requires_target=False,
            tags=["salesforce", "crm", "churn", "customer_success"],
        )
    except ImportError:
        pass
    ...
    try:
        ...
        registry["salesforce_nba"] = EngineInfo(
            name="salesforce_nba",
            display_name="Salesforce Next Best Action",
            description="AI-powered action recommendations for sales",
            category=EngineCategory.FINANCIAL,
            engine_class=SalesforceNBAEngine,
            priority=181,
            requires_target=False,
            tags=["salesforce", "crm", "nba", "recommendations"],
        )
    except ImportError:
        pass

    # Salesforce Deal Velocity
    try:
        from .salesforce_velocity_engine import SalesforceVelocityEngine

        registry["salesforce_velocity"] = EngineInfo(
            name="salesforce_velocity",
            display_name="Deal Velocity Analysis",
            description="Pipeline speed analytics and close date prediction",
            category=EngineCategory.FINANCIAL,
            engine_class=SalesforceVelocityEngine,
            priority=182,
            requires_target=False,
            tags=["salesforce", "crm", "velocity", "pipeline"],
        )
    except ImportError:
        pass
    ...
    try:
        ...
        registry["salesforce_competitive"] = EngineInfo(
            name="salesforce_competitive",
            display_name="Competitive Intelligence",
            description="Win/loss analysis and competitive positioning",
            category=EngineCategory.FINANCIAL,
            engine_class=SalesforceCompetitiveEngine,
            priority=183,
            requires_target=False,
            tags=["salesforce", "crm", "competitive", "analysis"],
        )
    except ImportError:
        pass
    ...
    try:
        ...
        registry["salesforce_c360"] = EngineInfo(
            name="salesforce_c360",
            display_name="Customer 360 Health",
            description="Unified customer health across all touchpoints",
            category=EngineCategory.FINANCIAL,
            engine_class=SalesforceC360Engine,
            priority=184,
            requires_target=False,
            tags=["salesforce", "crm", "customer_360", "health"],
        )
    except ImportError:
        pass
...
def run_single_engine(
    engine_info: EngineInfo, df: pd.DataFrame, target_column: str | None = None, config: dict[str, Any] | None = None
) -> EngineResult:
    """
    ...
    try:
        ...
        return EngineResult(engine_name=engine_info.name, success=True, duration_seconds=duration, result=result)
        ...
        return EngineResult(engine_name=engine_info.name, success=False, duration_seconds=duration, error=str(e))
...
def run_engines_in_order(
    ...
    engines_to_run = sorted(ENGINE_REGISTRY.values(), key=lambda e: e.priority)
    ...
    engines_to_run = [e for e in engines_to_run if e.enabled and e.name not in skip_engines]
...
async def run_engines_in_order_async(
    ...
    if not parallel:
        ...
        return run_engines_in_order(df, target_column, skip_engines, only_engines, config, verbose)
    ...
    engines_to_run = sorted(
        [e for e in ENGINE_REGISTRY.values() if e.enabled and e.name not in skip_engines], key=lambda e: e.priority
    )
    ...
    async def run_with_semaphore(engine_info: EngineInfo) -> EngineResult:
        async with semaphore:
            ...
            engine_config = config.get(engine_info.name, {})
            return await loop.run_in_executor(None, run_single_engine, engine_info, df, target_column, engine_config)
    ...
    for engine_info, result in zip(engines_to_run, engine_results):
        if isinstance(result, Exception):
            results[engine_info.name] = EngineResult(
                engine_name=engine_info.name, success=False, duration_seconds=0, error=str(result)
            )
            failed += 1
        else:
            results[engine_info.name] = result
...
def get_engine_info(engine_name: str) -> EngineInfo | None:
