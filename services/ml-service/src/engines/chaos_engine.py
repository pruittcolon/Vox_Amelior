...
CHAOS_CONFIG_SCHEMA = {
    "mi_threshold": {
        ...
        "max": 1.0,
        "default": 0.4,
        "description": "Mutual information threshold for hidden link detection",
    },
    "pearson_threshold": {
        "type": "float",
        "min": 0.1,
        "max": 0.9,
        "default": 0.6,
        "description": "Pearson threshold below which relationship is considered 'hidden'",
    },
    "n_top_links": {
        "type": "int",
        "min": 1,
        "max": 50,
        "default": 10,
        "description": "Number of top hidden links to return",
    },
}
...
class ChaosEngine:
    ...
    def analyze(
        ...
        hidden_links = self._find_hidden_links(numeric_cols, pearson_matrix, dcor_matrix, mi_matrix)

        # 5. Generate insights
        insights = self._generate_insights(numeric_cols, pearson_matrix, dcor_matrix, mi_matrix, hidden_links)
        ...
        return {
            ...
            "insights": insights,
            "complexity_matrix": {"columns": numeric_cols, "dcor": dcor_matrix, "mi": mi_matrix},
        }
    ...
    def _find_hidden_links(
        self, cols: list[str], pearson: dict[str, float], dcor: dict[str, float], mi: dict[str, float]
    ) -> list[dict[str, Any]]:
        """
    ...
    def _build_feature_importance_from_correlations(
        self, columns: list[str], mi_matrix: dict[str, float], pearson_matrix: dict[str, float]
        ...
        for col in columns:
            ...
            features.append(
                FeatureImportance(
                    name=col, stability=avg_mi * 100, importance=avg_mi, impact=impact, explanation=explanation
                )
            )
    ...
    def _build_explanation(self) -> TechnicalExplanation:
        ...
        return TechnicalExplanation(
            ...
            steps=[
                ...
                ExplanationStep(
                    3, "Distance Correlation", "Computed dCor to detect ANY dependency (linear + non-linear)"
                ),
                ExplanationStep(4, "Mutual Information", "Used information-theoretic measure to quantify dependency"),
                ExplanationStep(
                    5,
                    "Hidden Link Detection",
                    "Found pairs with high MI/dCor but low Pearson (hidden non-linear patterns)",
                ),
            ],
            limitations=[
...
if __name__ == "__main__":
    ...
    for link in result["hidden_links"]:
        ...
        print(
            f"  MI: {link['mutual_information']:.3f} | dCor: {link['distance_correlation']:.3f} | Pearson: {link['pearson_correlation']:.3f}"
        )
