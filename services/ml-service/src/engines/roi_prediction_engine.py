...
class ROIPredictionEngine:
    ...
    def get_engine_info(self) -> dict[str, str]:
        ...
        return {"name": "roi_prediction", "display_name": "ROI Prediction", "icon": "📊", "task_type": "prediction"}
            ...
            "steps": [
                {
                    ...
                    "description": "Identify investment and return columns",
                },
                {
                    "step_number": 2,
                    "title": "ROI Calculation",
                    "description": "Calculate basic and advanced ROI metrics",
                },
                {
                    "step_number": 3,
                    "title": "NPV Analysis",
                    "description": "Compute Net Present Value with discount rate",
                },
                {
                    "step_number": 4,
                    "title": "Sensitivity Analysis",
                    "description": "Assess ROI sensitivity to key variables",
                },
            ],
    ...
    def analyze(self, df: pd.DataFrame, config: dict | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if not config.get("skip_profiling", False):
                ...
                if investment_col:
                    results["column_mappings"]["investment"] = {"column": investment_col, "confidence": 0.9}
                if return_col:
                    results["column_mappings"]["returns"] = {"column": return_col, "confidence": 0.9}
                if time_col:
                    results["column_mappings"]["temporal"] = {"column": time_col, "confidence": 0.95}
            else:
            ...
            if not (investment_col and return_col):
                ...
                return {
                    ...
                    "insights": [
                        f"📊 **ROI Prediction - Not Applicable**: This engine requires financial data columns (investment/cost and return/revenue). Missing: {', '.join(missing)}. This dataset does not contain the necessary financial metrics for ROI analysis."
                    ],
            ...
            # Generate insights
            results["insights"] = self._generate_insights(results, investment_col, return_col, time_col)
    ...
    def _calculate_basic_roi(self, df: pd.DataFrame, investment_col: str, return_col: str) -> dict:
        ...
        df["roi"] = ((df[return_col] - df[investment_col]) / df[investment_col] * 100).replace(
    ...
    def _calculate_payback_period(self, df: pd.DataFrame, investment_col: str, return_col: str, time_col: str) -> dict:
        ...
        return {"payback_period": payback_period, "payback_achieved": len(breakeven_idx) > 0, "graph": graph}

    def _npv_analysis(
        self, df: pd.DataFrame, investment_col: str, return_col: str, time_col: str, discount_rate: float
    ) -> dict:
    ...
    def _calculate_irr(self, cash_flows: np.ndarray, max_iterations: int = 100) -> float | None:
    ...
    def _sensitivity_analysis(
        self, df: pd.DataFrame, investment_col: str, return_col: str, base_discount_rate: float
    ) -> dict:
        ...
        return {"discount_rates": discount_rates.tolist(), "npv_values": npvs, "graph": graph}
    ...
    def _generate_insights(
        self, results: dict, investment_col: str = None, return_col: str = None, time_col: str = None
    ) -> list[str]:
        """Generate insights"""
        insights = []
        ...
        if "risk" in results:
            risk = results["risk"]
            insights.append(f"🎯 Risk Level: {risk['risk_level']} | Success Rate: {risk['success_rate']:.1f}%")
