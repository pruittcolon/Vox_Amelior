...
class BudgetVarianceEngine:
    ...
    def get_methodology_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "steps": [
                ...
                {"step_number": 2, "title": "Variance Calculation", "description": "Calculate absolute and percentage variances"},
                {"step_number": 3, "title": "Root Cause Analysis", "description": "Identify drivers of budget variance"},
        ...
        try:
            ...
            if not config.get('skip_profiling', False):
                ...
                budget_col = self._smart_detect(
                    df, profiles,
                    keywords=['budget', 'planned', 'target', 'forecast'],
                    hint=config.get('budget_column')
                )
                ...
                budget_col = config.get('budget_column') or self._detect_column(df, ['budget', 'planned', 'target'], None)
                actual_col = config.get('actual_column') or self._detect_column(df, ['actual', 'spend', 'cost'], None)
                category_col = config.get('category_column') or self._detect_column(df, ['category', 'department'], None)
    ...
    def _category_variance(self, df: pd.DataFrame, budget_col: str, actual_col: str, category_col: str) -> Dict:
        ...
        waterfall_y = [grouped[budget_col].sum()] + grouped.head(5)['variance'].tolist() + [others_variance, grouped[actual_col].sum()]
        ...
        return {
            'variances': variances,
            'graph': graph,
            'waterfall_graph': waterfall_graph
        }
    
    def _comparison_chart(self, df: pd.DataFrame, budget_col: str, actual_col: str, category_col: Optional[str]) -> Dict:
    ...
    def _generate_insights(self, results: Dict) -> List[str]:
        ...
        if summary['total_variance'] > 0:
            insights.append(
                f"⚠️ Over Budget by ${summary['total_variance']:,.2f} "
                f"({summary['variance_percentage']:.1f}%)"
            )
        else:
            insights.append(
        ...
        if results['variances']:
            ...
            insights.append(
                f"🔍 Largest driver: {top_var['category']} with "
                f"${top_var['variance']:,.2f} variance"
            )
