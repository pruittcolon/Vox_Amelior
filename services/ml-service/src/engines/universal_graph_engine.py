...
class UniversalGraphEngine:
    ...
    def get_engine_info(self) -> dict[str, str]:
        ...
        return {"name": "graphs", "display_name": "Universal Graphs", "icon": "📊", "task_type": "detection"}
            ...
            "steps": [
                {
                    ...
                    "description": "Analyze column types, cardinality, and distributions",
                },
                {
                    "step_number": 2,
                    "title": "Graph Selection",
                    "description": "Choose appropriate chart types based on data characteristics",
                },
                {
                    "step_number": 3,
                    "title": "Data Preparation",
                    "description": "Sample large datasets and prepare data for visualization",
                },
                {
                    "step_number": 4,
                    "title": "Visualization Generation",
                    "description": "Create 10+ charts covering distributions, correlations, and trends",
                },
            ],
    ...
    def generate_graphs(self, df: pd.DataFrame, config: dict | None = None) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "metadata": {"rows": len(df), "columns": len(df.columns), "sampled": len(df) != len(df_sample)},
        }
    ...
    def _profile_dataset(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        missing_summary = {
            col: {"count": int(df[col].isna().sum()), "percentage": float(df[col].isna().sum() / len(df) * 100)}
            ...
            graphs.append(
                {
                    ...
                    "data": {"column": col, "values": df[col].dropna().tolist()[:1000]},
                    "priority": 7,
            ...
            graphs.append(
                {
                    ...
                    "data": {"matrix": corr_matrix, "columns": profile["numeric_columns"]},
                    "priority": 9,
    ...
    def _generate_missing_data_graphs(self, df: pd.DataFrame, profile: dict) -> list[dict]:
        ...
        if profile["missing_data"]:
            missing_data = {col: data["percentage"] for col, data in profile["missing_data"].items()}
            ...
            graphs.append(
                {
                    ...
                    "data": {"pivot": pivot.to_dict(), "col1": col1, "col2": col2},
                    "priority": 7,
