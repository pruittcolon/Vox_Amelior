...
SCOUT_CONFIG_SCHEMA = {
    "window_size": {
        ...
        "options": ["1D", "1W", "1M", "1Q"],
        "default": "1M",
        "description": "Time window size for drift detection",
    },
    "baseline_period": {
        "type": "int",
        "min": 1,
        "max": 12,
        "default": 1,
        "description": "Number of initial windows to use as baseline",
    },
    "psi_threshold": {
        "type": "float",
        "min": 0.05,
...
class ScoutEngine:
    ...
    def analyze(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not time_col:
            return {
                ...
                "insights": [
                    "📊 **Scout Engine - Not Applicable**: This engine requires a time/date column for drift detection. Dataset does not contain temporal data."
                ],
            }
        ...
        if len(df) < 100:
            return {
                ...
                "insights": [
                    f"📊 **Scout Engine - Insufficient Data**: Only {len(df)} valid timestamps found. Drift detection requires at least 100 temporal records for statistical significance."
                ],
            }
        ...
        if len(windows) < 3:
            return {
                ...
                "insights": [
                    f"📊 **Scout Engine - Insufficient Time Windows**: Only {len(windows)} time windows detected. Drift detection requires at least 3 windows for baseline comparison. Try using a smaller window size (current: {window_size})."
                ],
            }
        ...
        drift_events, feature_drift_scores = self._detect_drift(windows, numeric_cols, baseline_period)
        ...
        insights = self._generate_insights(
            time_col, windows, drift_events, feature_drift_scores, psi_scores, window_size
        )
        ...
        return {
            ...
            "date_range": {"start": str(df[time_col].min()), "end": str(df[time_col].max())},
    ...
    def _detect_drift(
        self, windows: list[pd.DataFrame], numeric_cols: list[str], baseline_period: int
    ...
    def _calculate_psi(
        ...
        baseline_data = pd.concat(windows[:baseline_period], ignore_index=True)
        current_data = (
            pd.concat(windows[baseline_period:], ignore_index=True) if len(windows) > baseline_period else baseline_data
        )
    ...
    def _generate_insights(
        ...
        if drift_events:
            ...
            for i, event in enumerate(drift_events[:3], 1):
                ...
                insights.append(f"   {i}. **{period}** ({severity} severity): {n_features} feature(s) drifted")
    ...
    def run_premium(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> PremiumResult:
        ...
        variants = self._convert_drift_events_to_variants(
            raw_result.get("drift_events", []), raw_result.get("psi_scores", {})
        )
        ...
        feature_importance = self._convert_psi_to_importance(
            raw_result.get("psi_scores", {}), raw_result.get("time_column", "")
        )
    ...
    def _convert_drift_events_to_variants(
        self, drift_events: list[dict], psi_scores: dict[str, float]
    ) -> list[Variant]:
        """Convert drift events to variants"""
        variants = []
    ...
    def _convert_psi_to_importance(self, psi_scores: dict[str, float], time_col: str) -> list[FeatureImportance]:
    ...
    def _error_to_premium_result(
        ...
        message = (
            raw_result.get("message", "") or raw_result.get("insights", [""])[0]
...
if __name__ == "__main__":
    ...
    engine = ScoutEngine()
    config = {"time_column": time_col, "window_size": window_size}
