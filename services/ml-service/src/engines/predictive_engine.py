...
class PredictiveEngine:
            ...
            "steps": [
                {
                    ...
                    "description": "Identify temporal column for time series",
                },
                {"step_number": 2, "title": "Stationarity Testing", "description": "Check for trends and seasonality"},
                {
                    "step_number": 3,
                    "title": "Model Selection",
                    "description": "Auto-select best model via cross-validation",
                },
                {
                    "step_number": 4,
                    "title": "Forecasting",
    ...
    def get_requirements(self) -> EngineRequirements:
        return EngineRequirements(
            required_semantics=[SemanticType.TEMPORAL, SemanticType.NUMERIC_CONTINUOUS], min_rows=50, min_numeric_cols=1
        )
    ...
    def forecast(self, df: pd.DataFrame, config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "model_comparison": {name: {"mae": score} for name, score in model_scores.items()},
            "visualizations": self._generate_forecast_visualizations(best_result, ts_data),
            "metadata": {"horizon": horizon, "data_points": len(ts_data), "models_tried": list(models_to_try)},
        }
    ...
    def _naive_forecast(self, ts_data: pd.Series, horizon: int) -> dict:
        ...
        return {
            ...
            "model": "naive",
            "validation_metrics": {"mae": float(mae), "rmse": float(rmse)},
        }
        ...
        visualizations.append(
            {
                ...
                "data": {"MAE": metrics.get("mae", 0), "RMSE": metrics.get("rmse", 0)},
