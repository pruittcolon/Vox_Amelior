...
class SpendPatternEngine:
    ...
    def get_config_schema(self) -> list[ConfigParameter]:
        ...
        return [
            ConfigParameter(
                name="spend_column", type="select", default=None, range=[], description="Column containing spend values"
            ),
            ConfigParameter(
                name="date_column", type="select", default=None, range=[], description="Column containing dates"
            ),
            ConfigParameter(
                name="category_column", type="select", default=None, range=[], description="Column for spend categories"
            ),
        ]
    ...
    def get_methodology_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "steps": [
                {
                    "step_number": 1,
                    "title": "Spend Detection",
                    "description": "Identify spending columns via semantic intelligence",
                },
                {"step_number": 2, "title": "Temporal Analysis", "description": "Analyze spending patterns over time"},
                {
                    "step_number": 3,
                    "title": "Anomaly Detection",
                    "description": "Identify unusual spending using Isolation Forest",
                },
    ...
    def analyze(self, df: pd.DataFrame, config: dict | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if not config.get("skip_profiling", False):
                ...
                date_col = config.get("date_column") or self._detect_column(df, ["date", "time", "timestamp"], None)
            ...
            if category_col:
                ...
                try:
                    ...
                    results["category_totals"] = {str(cat): float(val) for cat, val in cat_totals.items()}
            ...
            # Generate insights
            results["insights"] = self._generate_insights(results, spend_col, date_col, category_col, supplier_col)
    ...
    def _category_breakdown(self, df: pd.DataFrame, spend_col: str, category_col: str) -> dict:
        ...
        return {"top_categories": category_spend.head(10).to_dict("records"), "graph": graph}
    ...
    def _detect_spending_anomalies(self, df: pd.DataFrame, spend_col: str, category_col: str | None) -> dict:
    ...
    def _detect_maverick_spend(
        self, df: pd.DataFrame, spend_col: str, supplier_col: str, contract_col: str | None
    ) -> dict:
    ...
    def _generate_insights(
        ...
        if "summary" in results:
            ...
            insights.append(
                f"💵 Total Spend: ${summary['total_spend']:,.2f} across {summary['transaction_count']} transactions"
            )
        ...
        if "patterns" in results and "temporal" in results["patterns"]:
            ...
            insights.append(
                f"{trend_emoji} Spending trend: {temporal['trend']} (strength: {temporal['trend_strength']:.2f})"
            )

        if "anomalies" in results and results["anomalies"]:
            insights.append(
                f"⚠️ Detected {len(results['anomalies'])} anomalous transactions - recommend review for errors or fraud"
            )
