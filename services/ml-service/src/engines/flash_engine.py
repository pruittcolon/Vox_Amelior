...
FLASH_CONFIG_SCHEMA = {
    ...
    "desired_class": {
        "type": "select",
        "options": ["opposite", "1", "0"],
        "default": "opposite",
        "description": "Target class for counterfactuals",
    },
}
...
class FlashEngine:
    ...
    def analyze(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        counterfactuals = self._generate_counterfactuals(
            df, target_col, feature_names, continuous_features, model, num_cfs, desired_outcome, query_instances
        )

        # 7. Generate insights
        insights = self._generate_insights(target_col, counterfactuals, desired_outcome, accuracy)
    ...
    def _prepare_data(self, df: pd.DataFrame, target_col: str) -> tuple[np.ndarray, np.ndarray, list[str], list[str]]:
        ...
        for col in feature_cols:
            if col not in numeric_features and df[col].dtype == "object":
                ...
                if len(unique_vals) <= 2 and set(str(v).lower() for v in unique_vals) <= {
                    "true",
                    "false",
                    "1",
                    "0",
    ...
    def _generate_counterfactuals(
        ...
        d = dice_ml.Data(dataframe=feature_df, continuous_features=continuous_features, outcome_name=target_col)
        ...
        for idx in undesired_indices:
            try:
                ...
                dice_exp = exp.generate_counterfactuals(
                    query_instance, total_CFs=num_cfs, desired_class=desired_outcome
                )
    ...
    def _generate_insights(
        self, target_col: str, counterfactuals: list[dict], desired_outcome: int, accuracy: float
    ) -> list[str]:
        ...
        # Model performance
        insights.append(
            f"✅ **Model Accuracy**: {accuracy * 100:.1f}%. Counterfactuals based on Random Forest predictions."
        )
        ...
        if counterfactuals:
            ...
            insights.append("📈 **Key Intervention Levers**: Most frequently changed features across counterfactuals:")
            ...
            if len(counterfactuals) > 0:
                ...
                for feat, change in list(example["changes"].items())[:3]:
                    insights.append(
                        f"   • Change '{feat}' from {change['from']:.2f} → {change['to']:.2f} (Δ{change['delta']:+.2f})"
                    )

                insights.append(f"   ➜ Result: '{target_col}' changes to {desired_outcome}")
        else:
            insights.append(
    ...
    def run_premium(self, df: pd.DataFrame, config: dict[str, Any] | None = None) -> PremiumResult:
        ...
        for i, cf in enumerate(cfs[:10]):
            variants.append(
                Variant(
                    rank=i + 1,
                    gemma_score=90 - i * 5,
                    cv_score=1 / (cf.get("num_features_changed", 1) + 1),
        ...
        features = [
            FeatureImportance(
                name=f,
                stability=80.0,
                importance=count / max(1, len(cfs)),
                impact="mixed",
                explanation=f"{f} changed in {count} counterfactual(s)",
            )
        ...
        return PremiumResult(
            engine_name="flash",
            engine_display_name="Flash What-If",
            engine_icon="⚡",
            ...
            summary=PlainEnglishSummary(
                ...
                "DiCE generated counterfactual explanations showing minimal changes to flip predictions.",
            ...
            config_schema=[
                ...
                for k, v in FLASH_CONFIG_SCHEMA.items()
            ],
    ...
    def _error_to_premium_result(self, raw, df, config, start):
        ...
        return PremiumResult(
            ...
            feature_importance=[],
            summary=PlainEnglishSummary(
                "What-if analysis failed", raw.get("message", "Error"), "Provide classification data.", Confidence.LOW
            ),
