...
class GPUClient:
    ...
    def __init__(
        ...
        """
        self.coordinator_url = coordinator_url or os.getenv("GPU_COORDINATOR_URL", "http://gpu-coordinator:8002")
    ...
    def _get_service_headers(self) -> dict[str, str]:
        ...
        headers = {"X-Service-ID": "ml-service", "X-Request-Source": "ml-engine"}
    ...
    async def _get_http_client(self) -> httpx.AsyncClient:
        ...
        if self._http_client is None or self._http_client.is_closed:
            self._http_client = httpx.AsyncClient(
                timeout=httpx.Timeout(self.timeout_seconds), limits=httpx.Limits(max_connections=5)
            )
    ...
    async def check_coordinator_health(self) -> bool:
        ...
        try:
            ...
            response = await client.get(f"{self.coordinator_url}/health", timeout=5.0)
    ...
    async def request_gpu(
        self, engine_name: str, priority: GPURequestPriority = GPURequestPriority.IMMEDIATE
        ...
        if self._acquired:
            ...
            return GPUAcquisitionResult(acquired=True, task_id=self._current_task_id)
        ...
        for attempt in range(self.retry_attempts + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"[GPU] ✅ Acquired GPU for {engine_name} (task_id={task_id}, time={acquisition_time:.1f}ms)"
                    )

                    return GPUAcquisitionResult(acquired=True, task_id=task_id, acquisition_time_ms=acquisition_time)
                else:
        ...
        return GPUAcquisitionResult(acquired=False, error=error_msg, acquisition_time_ms=acquisition_time)
    ...
    async def release_gpu(self) -> bool:
        ...
        try:
            ...
            response = await client.post(
                f"{self.coordinator_url}/gemma/release/{task_id}", json={}, headers=self._get_service_headers()
            )
    ...
    @asynccontextmanager
    async def gpu_context(self, engine_name: str, priority: GPURequestPriority = GPURequestPriority.IMMEDIATE):
        """
