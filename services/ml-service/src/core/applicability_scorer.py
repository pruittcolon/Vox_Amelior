...
from .dataset_classifier import BusinessDomain, DatasetClassification, MLTaskType
...
class ApplicabilityScorer:
    ...
    def _define_requirements(self) -> dict[AnalyticsEngine, EngineRequirements]:
        ...
        return {
            ...
            AnalyticsEngine.CUSTOMER_LTV: EngineRequirements(
                ...
                optional_semantics={"monetary": [SemanticType.NUMERIC_CONTINUOUS], "temporal": [SemanticType.TEMPORAL]},
    ...
    def score_engines(
        self, profiles: dict[str, ColumnProfile], classification: DatasetClassification
    ) -> list[EngineScore]:
        """
    ...
    def _score_engine(
        ...
        for req_semantic in requirements.required_semantics:
            matching_cols = [col for col, prof in profiles.items() if prof.semantic_type == req_semantic]
            if matching_cols:
    ...
    def _generate_reason(
        self, engine: AnalyticsEngine, score: float, missing: list[str], classification: DatasetClassification
    ) -> str:
        ...
        if score > 0.8:
            ...
            return "Possible match. May have limited results"
        else:
...
def get_top_recommendations(
    profiles: dict[str, ColumnProfile], classification: DatasetClassification, top_n: int = 5
) -> list[EngineScore]:
    """
