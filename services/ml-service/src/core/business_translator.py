...
class BusinessTranslator:
    ...
    def generate_summary(
        ...
        if ratio >= 0.5:
            ...
            feature_quality = f"Found {stable_features} stable features - consider engineering more features for better results."
        else:
            feature_quality = f"Only {stable_features} of {total_features} features are statistically robust - more data may help."
        
        return f"The {engine_info['display_name']} model {performance} when predicting {clean_target}. {feature_quality}"
