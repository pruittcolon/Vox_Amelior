...
class GemmaSummarizer:
    ...
    def generate_fallback_summary(
        ...
        return {
            ...
            'recommendations': gemma_summary.get('recommendations', [
                {
    ...
    @classmethod
    def _get_column_hints(cls, engine_name: str, columns: List[str]) -> Optional[str]:
...
Keep response under 150 words. Be direct and specific."""
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    'recommendations': [{
                        'action': 'Follow Gemma suggestions',
    ...
    def _prepare_data_description(cls, df: pd.DataFrame) -> str:
        ...
        if numeric_cols:
            col = numeric_cols[0]
            lines.append(f"- Sample numeric ({col}): mean={df[col].mean():.2f}, range=[{df[col].min():.2f}, {df[col].max():.2f}]")
