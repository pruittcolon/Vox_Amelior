...
class ColumnProfiler:
    ...
    def profile_column(self, series: pd.Series) -> ColumnProfile:
        ...
        if semantic_type in [SemanticType.NUMERIC_CONTINUOUS, SemanticType.NUMERIC_DISCRETE]:
            ...
            if len(numeric_series) > 0:
                ...
                IQR = Q3 - Q1
                has_outliers = ((numeric_series < (Q1 - 1.5 * IQR)) | 
                               (numeric_series > (Q3 + 1.5 * IQR))).any()
    ...
    def _detect_semantic_type(self, series: pd.Series, uniqueness: float) -> Tuple[SemanticType, float]:
        ...
        if pd.api.types.is_numeric_dtype(series):
            ...
            value_keywords = ['cost', 'price', 'amount', 'revenue', 'sales', 'spend', 
                            'expense', 'payment', 'value', 'total', 'sum', 'profit']
    ...
    def _detect_business_entity(self, col_name: str, series: pd.Series, 
                                semantic_type: SemanticType) -> BusinessEntity:
        """
