...
class ColumnMapper:
    ...
    def map_columns(self, 
                   ...
                   requirements: EngineRequirements) -> MappingResult:
        """
        ...
        for req_semantic in requirements.required_semantics:
            ...
            mapping = self._find_best_match(
                role, req_semantic, None, profiles, is_required=True
            )
        ...
        for req_entity in requirements.required_entities:
            ...
            mapping = self._find_best_match(
                role, None, req_entity, profiles, is_required=True
            )
    ...
    def _find_best_match(self,
                        ...
                        profiles: Dict[str, ColumnProfile],
                        is_required: bool) -> Optional[ColumnMapping]:
        """
        ...
        # Filter by business entity
            ...
            candidates = [
                (col, prof) for col, prof in profiles.items()
                if prof.detected_entity == business_entity
            ]
        ...
        if not candidates:
            ...
            candidates = [
                (col, prof) for col, prof in profiles.items()
                if self._fuzzy_match(role, col) >= self.fuzzy_threshold
            ]
    ...
    def resolve_ambiguity(self,
                         ...
                         resolutions: Dict[str, str]) -> MappingResult:
        """
        ...
        remaining_ambiguous = [
            (role, candidates) for role, candidates in mapping_result.ambiguous
        ...
        return MappingResult(
            ...
            confidence=confidence,
            message="Ambiguities resolved" if not remaining_ambiguous else f"{len(remaining_ambiguous)} ambiguities remaining"
        )


def auto_map_columns(profiles: Dict[str, ColumnProfile],
                    requirements: EngineRequirements) -> Dict[str, str]:
    """
