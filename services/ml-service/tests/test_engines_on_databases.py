...
def main():
    ...
    results.append((
        "UniversalGraph on users.db/users",
        test_universal_graph_on_database("users.db", users_db_path, "users")
    ))
    ...
    results.append((
        "UniversalGraph on email.db",
        test_universal_graph_on_database("email.db", email_db_path, "emails")
    ))
