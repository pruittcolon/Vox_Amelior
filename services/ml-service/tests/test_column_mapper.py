...
from core.applicability_scorer import EngineRequirements
from core.column_mapper import ColumnMapper, auto_map_columns
from core.schema_intelligence import BusinessEntity, ColumnProfiler, SemanticType
...
class TestColumnMapper(unittest.TestCase):
    ...
    def test_exact_semantic_match(self):
        ...
        df = pd.DataFrame(
            {"total_cost": np.random.uniform(100, 1000, 50), "category": np.random.choice(["A", "B", "C"], 50)}
        )
    ...
    def test_business_entity_match(self):
        ...
        df = pd.DataFrame(
            {"product_cost": np.random.uniform(10, 100, 50), "item_name": ["Item" + str(i) for i in range(50)]}
        )
    ...
    def test_missing_required_field(self):
        ...
        df = pd.DataFrame({"description": ["text" + str(i) for i in range(50)]})
    ...
    def test_ambiguity_resolution(self):
        ...
        df = pd.DataFrame({"revenue": np.random.uniform(1000, 5000, 50), "cost": np.random.uniform(500, 2000, 50)})
    ...
    def test_confidence_scoring(self):
        ...
        df = pd.DataFrame({"amount": np.random.uniform(100, 1000, 50), "type": np.random.choice(["A", "B"], 50)})
