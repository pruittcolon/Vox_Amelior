...
def extract_audio_metrics(audio_data: np.ndarray, sample_rate: int, text: str | None = None) -> dict[str, float]:
    """
    ...
    try:
        ...
        if LIBROSA_AVAILABLE:
            try:
                ...
                spectral_centroids = librosa.feature.spectral_centroid(y=audio_data.astype(np.float32), sr=sample_rate)[
                    0
                ]
                ...
                spectral_rolloff = librosa.feature.spectral_rolloff(y=audio_data.astype(np.float32), sr=sample_rate)[0]
                metrics["spectral_rolloff_mean"] = float(np.mean(spectral_rolloff))

                # MFCC (Mel-frequency cepstral coefficients) - voice timbre
                mfccs = librosa.feature.mfcc(y=audio_data.astype(np.float32), sr=sample_rate, n_mfcc=13)
...
def calculate_segment_metrics(
    audio_data: np.ndarray, sample_rate: int, start_time: float, end_time: float, text: str
) -> dict[str, float]:
    """
