...
class GPUMonitor:
    ...
    def get_vram_usage(self) -> dict[str, int] | None:
        """
    ...
    def get_gpu_utilization(self) -> int | None:
        """
        ...
        try:
            result = subprocess.run(
                ["nvidia-smi", f"--id={self.gpu_id}", "--query-gpu=utilization.gpu", "--format=csv,noheader,nounits"],
                capture_output=True,
                text=True,
                check=True,
                timeout=2,
        ...
        try:
            ...
            for line in result.stdout.strip().split("\n"):
                if line.strip():
                    ...
                    if len(parts) >= 3:
                        processes.append(
                            {"pid": parts[0].strip(), "name": parts[1].strip(), "used_memory_mb": parts[2].strip()}
                        )
