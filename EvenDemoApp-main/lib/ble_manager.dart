...
class BleManager {
  ...
  void startListening() {
    print('🔧 [DEBUG] BleManager.startListening() called');
    eventBleReceive.listen((res) {
      _handleReceivedData(res);
    });
    print('🔧 [DEBUG] BleManager event listener started');
  }

  Future<void> startScan() async {
    try {
      print('🔧 [DEBUG] BleManager.startScan() called');
      await _channel.invokeMethod('startScan');
      print('🔧 [DEBUG] BleManager.startScan() completed successfully');
    } catch (e) {
      print('❌ [ERROR] Error starting scan: $e');
    }
  }

  Future<void> stopScan() async {
    try {
      print('🔧 [DEBUG] BleManager.stopScan() called');
      await _channel.invokeMethod('stopScan');
      print('🔧 [DEBUG] BleManager.stopScan() completed successfully');
    } catch (e) {
      print('❌ [ERROR] Error stopping scan: $e');
    }
  }
  ...
  void setMethodCallHandler() {
    print('🔧 [DEBUG] BleManager.setMethodCallHandler() called');
    _channel.setMethodCallHandler(_methodCallHandler);
    print('🔧 [DEBUG] BleManager method call handler set');
  }
  ...
  // MODIFIED: Handle the "VoiceChunk" type from the native side
  void _handleReceivedData(BleReceive res) {
    // If the data is an audio chunk, pass it directly to the EvenAI service.
    if (res.type == "VoiceChunk") {
      EvenAI.get.onVoiceChunkReceived(res.data);
      return;
    }

    // --- The rest of the function handles regular command data ---
