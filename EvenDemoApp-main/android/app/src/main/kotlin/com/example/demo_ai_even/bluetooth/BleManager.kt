...
class BleManager private constructor() {
    ...
    // MODIFIED: Add a WakeLock instance to prevent CPU from sleeping
    ...
    fun initBluetooth(context: Activity) {
        ...
        // MODIFIED: Initialize PowerManager and WakeLock
        val powerManager = context.getSystemService(Context.POWER_SERVICE) as PowerManager
        wakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "EvenAI::WakelockTag")

        Log.v(LOG_TAG, "BleManager init success")
    }

    // MODIFIED: Method to acquire the WakeLock, called from BleChannelHelper
    fun acquireWakeLock() {
        if (wakeLock?.isHeld == false) {
            wakeLock?.acquire(30*60*1000L /*30 minutes timeout*/) // Set a reasonable timeout
            Log.d(LOG_TAG, "Wakelock acquired")
        }
    }

    // MODIFIED: Method to release the WakeLock, called from BleChannelHelper
    fun releaseWakeLock() {
    ...
    private fun bleGattCallBack(): BluetoothGattCallback = object : BluetoothGattCallback() {
        ...
        override fun onCharacteristicChanged(
            ...
            mainScope.launch {
                ...
                // MODIFIED: Decode LC3 to PCM and send the PCM data to Flutter
                if (isMicData) {
                    val lc3 = value.copyOfRange(2, 202)
                    val pcmData = Cpp.decodeLC3(lc3) // Decode to raw PCM audio
                    if (pcmData != null) {
                        BleChannelHelper.bleReceive(mapOf(
                            "lr" to if (isLeft)  "L" else "R",
                            "data" to pcmData, // Send the decoded PCM data
                            "type" to "VoiceChunk"
                        ))
                    }
                } else {
                    // If it's not mic data, send the original command data
                    BleChannelHelper.bleReceive(mapOf(
                        "lr" to if (isLeft)  "L" else "R",
                        "data" to value,
                        "type" to "Receive"
